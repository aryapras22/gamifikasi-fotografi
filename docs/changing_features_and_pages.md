...
1. **Locate the View**: Views are stored under [lib/views/](../gamifyphotography/lib/views/). Find the target folder (e.g., `views/mission/challenge_view.dart`).
...
1. **Create the View File**:
   Create a new file `lib/views/profile/settings_view.dart` inside the mobile project:
   ```dart
...
2. **Register the Route**:
   Open [lib/core/router.dart](../gamifyphotography/lib/core/router.dart). Add a new `GoRoute` to the routes array:
   ```dart
   GoRoute(
     path: '/settings',
...
1. **Locate the Feature**: Features are stored in [src/features/](../gamifyphotography-admin/src/features/). For example, to change the submissions grid, open `features/submissions/`.
...
1. **Create Feature Components**:
   Create a new directory `src/features/missions/` under `gamifyphotography-admin/` and add `MissionsPage.tsx`:
   ```tsx
...
2. **Register the Page in Router**:
   Open [src/app/router.tsx](../gamifyphotography-admin/src/app/router.tsx). Import your component and append the route configuration to the path list:
   ```tsx
...
3. **Add Navigation to Sidebar**:
   Open [src/app/layout.tsx](../gamifyphotography-admin/src/app/layout.tsx). Locate the sidebar navigation lists and add a link block:
   ```tsx
