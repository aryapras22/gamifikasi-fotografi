...
2. **Download Package Dependencies**:
   * The app relies on various third-party libraries (defined in [pubspec.yaml](../gamifyphotography/pubspec.yaml)). Run:
     ```bash
     flutter pub get
     ```
...
2. **Install Node Modules**:
   * Install the application dependencies specified in [package.json](../gamifyphotography-admin/package.json):
     ```bash
     npm install
     ```
...
1. The seeding blueprint structure and raw data are located in the mobile app's [seed_firestore.dart](../gamifyphotography/scripts/seed_firestore.dart).
